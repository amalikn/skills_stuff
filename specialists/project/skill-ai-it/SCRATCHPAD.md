...
**Phase:** Package now governs itself — `scripts/check_governance.py` (129 assertions), `justfile`, pinned runtimes, lint config, and `scripts/selftest_blocks.py`. Managed blocks are generated by
reading the templates; the inlined copies are gone. Current stamp is recorded in `scripts/README.md` under Coupled Constants, and `just check` fails if any of its twelve surfaces drifts.
...
- The upgrader's inlined navigation block had drifted nine sections behind `templates/AI_NAVIGATION.md` since 2026-08-11. Bootstrapping then upgrading a project deleted 105 lines of routing while
  reporting `replaced-managed-block`. All three builders now read their templates; a missing template stops the run instead of falling back.
- `scripts/selftest_blocks.py` added — 20 assertions, with the required-section floor stated independently of any template, because comparing the emitted block to the template it is generated from
  cannot fail.
- `scripts/check_governance.py` added. On its first run it caught that this package's OWN `scripts/README.md` had lost four sections to the self-upgrade, because they were nested inside the
...
Persisted by `/slurp-chat` at 20260923_1305 — memory-keeper channel `skills-stuff`, mcp-project-context project `skills_stuff`
(`b8c5525e-3e2f-4fb5-bf87-e5751f3ad49c`). Checkpoint `slurp-20260923-skill-ai-it-template-drift` in both backends
(project-context id `4c8a9164-7b60-4072-8390-5d7e91f5ae01`).

Keys written this session:

- `skills_stuff.skill-ai-it.nav-block-template-drift-20260923` — the inlined-block drift and how it hid for six weeks
- `skills_stuff.skill-ai-it.template-sourced-blocks-decision-20260923` — why the templates own block content and why there is no fallback copy
- `skills_stuff.skill-ai-it.scripts-readme-managed-block-loss-20260923` — this package losing its own file to its own upgrader, and the backup rule
- `skills_stuff.skill-ai-it.self-governance-20260923` — checker, self-test, justfile, pins, lint
