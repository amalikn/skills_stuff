...
| [SUPERSEDED — 0004 Sync is report-first](adr/0004-sync-is-report-first.md) | What upstream sync may apply without review                      |
...
| [SUPERSEDED — 0009 Sync apply is staged, then promoted](adr/0009-sync-apply-is-atomic.md) | The upstream sync transaction model                              |
...
| [SUPERSEDED — 0010 Sync refuses symlinks and enforces containment](rules/0010-sync-refuses-symlinks.md) | `sync_auto_company.py`                                 |
| [0011 Gate errors are asymmetric](rules/0011-gate-errors-are-asymmetric.md) | `evaluate_routing.py` scorer                           |
| [0012 Gate flags are advisory until the collapse is localised](rules/0012-gate-flags-are-advisory-until-localised.md) | Operator policy for current use                        |
| [0013 Trim skill/persona prose against the frozen corpus as a gate](rules/0013-trim-against-the-frozen-corpus-as-a-gate.md) | `evaluate_routing.py` pass/fail on hard invariants     |
...
| [0006 Runner qualification before evidence is spent](specs/0006-runner-qualification.md) | Evaluation pipeline reliability                         |
| [0007 Gate-only evaluation and precision/recall scoring](specs/0007-gate-only-evaluation.md) | Localising the gate collapse                            |
| [0008 Replay corpus contract](specs/0008-replay-corpus-contract.md) | Evidence rules for historical-task replay               |

## Guides

| Document                                    | Procedure                                         |
| ------------------------------------------- | ------------------------------------------------- |
| [SUPERSEDED — 0001 Upstream sync workflow](guides/0001-upstream-sync.md) | status, dry-run, fetch-dry-run, apply             |
...
| [0003 Holdout 2 protocol](plans/0003-holdout-two-protocol.md) | Approved; preconditions before a second blind corpus    |
...
| `docs/audits/audit-agent-stack.md`             | It is the audit **prompt**, not a second report. Never a duplicate and never a promotion candidate.                                                 |
| `CHANGELOG.md` entries                         | History, not truth. Explicitly excluded from candidate inspection.                                                                                  |
| ~~`upstream-state.json`,~~                     | Generated sync state. **Both files and the tool were removed 20260903** when upstream sync was retired; the row stays so a future scan does not     |
|   ~~`translation-memory.json`~~                |   re-propose them.                                                                                                                                  |
