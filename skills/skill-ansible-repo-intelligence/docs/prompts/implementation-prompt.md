...
`ANSIBLE_REPO_MAP.md` should primarily surface `critical` and `high` nodes.

All nodes remain queryable from `graph.yaml`.
...
- baseline agent workflow without generated context
- indexed workflow using `ANSIBLE_REPO_MAP.md`, bounded queries and source verification
...
Treat these as empirical acceptance targets, not guaranteed outcomes.

If the thresholds are not met, report `PARTIAL` and identify the cause.
