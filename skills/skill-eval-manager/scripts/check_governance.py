...
SURFACES: tuple[str, ...] = (
    ...
    "BRIEF.md",
    "scripts/README.md",
    "examples/recording-demo/README.md",
    "examples/validation-failures/README.md",
)
...
CATALOGS: dict[str, tuple[str, str]] = {
    # scripts/README.md is the operational catalog every agent is routed to before running anything here. A script that
    # is not in it has no recorded purpose, inputs, outputs, safety label or idempotency — which AGENTS.md requires.
    "scripts/README.md": ("scripts", "*.py"),
    # Each negative fixture must be described, or an agent cannot tell a deliberately invalid suite from a broken one.
    "examples/validation-failures/README.md": ("examples/validation-failures", "*.yaml"),
    # `.archcore/index.guide.md` is the durable promotion index, and the only surface that records what is deliberately
    # NEVER promoted. A document promoted but never indexed is invisible to the next promotion scan, which then
    # re-proposes what has already been settled. The glob is "*/*.md" so it covers adr/rules/specs/guides/plans and
    # excludes the index itself. Not ARCHCORE_PROMOTION_CANDIDATES.md: that file is transient and `promote` deletes it.
    ".archcore/index.guide.md": (".archcore", "*/*.md"),
    # SKILL.md's References section is how an agent finds the method file for a task, and step 1 of the standing
    # write-back contract routes through it. A reference file that exists but is not routed there is unreachable: the
    # contract tells a consuming project to write to "the matching references/ file" and nothing would point at it.
    # This is the only half of the write-back contract a checker can see; knowledge that never left the consuming
    # project is invisible from here, which AGENTS.md states rather than pretends otherwise.
...
CONDITIONAL_PATHS: frozenset[str] = frozenset({
    ...
    "Makefile",                       # no make target here
    "package.json",                   # no Node package; node is pinned only for JS CLI tooling
    "graphify-out/GRAPH_REPORT.md",   # generated; absent until `graphify update .` has run
    "graphify-out/graph.json",
    ".ai-context/governance-pack.md", # generated; absent until repomix has run
    ".ai-context/repo-pack.md",
    "memory-bank/activeContext.md",   # this project uses SCRATCHPAD.md, not a memory-bank
    "memory-bank/progress.md",
    "memory-bank/decisionLog.md",
    "ARCHITECTURE.md",                # design rationale lives in BRIEF.md and the docs/ design record
    "architecture.md",
    "ROADMAP.md",                     # forward work is tracked in BRIEF.md "Deferred to v0.2"
    "roadmap.md",
    # `archcore init` creates settings.json only; these subfolders appear on the first promotion. adr/, rules/ and
    # specs/ were populated by the 20260923_1311 promote run and their exemptions retired with it, so the navigation
    # block's route to them is now enforced. Remove each remaining entry the day its folder exists.
    ".archcore/guides",               # index.guide.md sits at the .archcore root; no guides/ folder yet
    ".archcore/plans",                # nothing approved to plan against; see .archcore/index.guide.md
    # Deleted by the 20260923_1311 promote run, as promote is defined to do. CHANGELOG.md still names it, and that
    # mention is history rather than a live route — the durable index is .archcore/index.guide.md.
    "ARCHCORE_PROMOTION_CANDIDATES.md",
    # Named by the regenerated managed navigation block, which must not be hand-edited:
    "docs/reports",                   # generic audit-output folder; this repo keeps none
    "report.md",                      # a shape in the managed block, not a path — real ones are examples/*/report.md
})
...
RUNNER_REFERENCES: tuple[str, ...] = ("scripts/README.md", "AGENTS.md", "AI_NAVIGATION.md")
...
DERIVED: dict[str, tuple[str, ...]] = {
    # SKILL.md quality gate: "History is append-only; corrections supersede rather than overwrite; reports are derived."
    # A checked-in report older than the suite or history it was rendered from is a scorecard asserting a state the
    # record no longer supports — the exact failure this package exists to prevent, committed by the package itself.
    # Regenerate with: python scripts/render_report.py --suite <suite> --history <history> --output <report>
    "examples/network-controller/report.md": (
        "examples/network-controller/evals.yaml",
        "examples/network-controller/history.jsonl",
    ),
    "examples/financial-reconciliation/report.md": (
        "examples/financial-reconciliation/evals.yaml",
        "examples/financial-reconciliation/history.jsonl",
    ),
    "examples/recording-demo/report.md": (
        "templates/evals.yaml",
        "examples/recording-demo/history.jsonl",
    ),
    "examples/recording-demo/invalidation-report.md": (
        "templates/evals.yaml",
        "examples/recording-demo/invalidation-history.jsonl",
    ),
}
...
CONSTANT_SURFACES: dict[str, dict[str, object]] = {
    # The package version is deliberately restated in three places: SKILL.md frontmatter (what a loading agent reads),
    # README.md (what a human reads), and the CHANGELOG heading (what shipped). Found drifted on 2026-09-23 — CHANGELOG
    # said 0.1.1 while both pointers still said 0.1.0, so the frontmatter advertised a build whose renderer defect had
...
def check_archcore_contract() -> None:
    """Every promoted .archcore document satisfies the frontmatter contract its own index declares.

    Cites `.archcore/index.guide.md` "Proposing another document", which states the filename shape and the five
    required keys. `archcore status` enforces only the FILENAME; the frontmatter is unchecked by the CLI, and a
    document whose `type:` disagrees with its filename mis-routes silently while both halves look right in isolation.
    `status` is a closed set because the index now treats "accepted" as authoritative over AGENTS.md and SKILL.md — an
    unrecognised value would leave a document's authority undefined rather than obviously wrong.
    """
    required = ("title", "type", "status", "tags", "provenance")
    allowed_status = {"proposed", "accepted", "superseded"}

    for doc in sorted(ROOT.glob(".archcore/*/*.md")) + sorted(ROOT.glob(".archcore/*.md")):
        rel = doc.relative_to(ROOT).as_posix()
        text = doc.read_text(encoding="utf-8", errors="ignore")
        counted()
        if not text.startswith("---\n"):
            fail("archcore", f"{rel} has no YAML frontmatter; see .archcore/index.guide.md")
            continue
        front = text.split("---\n", 2)[1]
        keys = {line.split(":", 1)[0].strip(): line.split(":", 1)[1].strip()
                for line in front.splitlines() if ":" in line and not line.startswith((" ", "\t", "-"))}

        for key in required:
            counted()
            if key not in keys:
                fail("archcore", f"{rel} frontmatter is missing `{key}`; see .archcore/index.guide.md")

        counted()
        if keys.get("status") not in allowed_status:
            fail("archcore", f"{rel} has status `{keys.get('status')}`; allowed: {', '.join(sorted(allowed_status))}")

        # `<slug>.<type>.md` -> the type segment must match the declared type, or routing and frontmatter disagree.
        parts = doc.name.split(".")
        counted()
        if len(parts) >= 3 and keys.get("type") != parts[-2]:
