# AI Navigation — skill-eval-manager
...
- [Package-specific routing — skill-eval-manager](#package-specific-routing--skill-eval-manager)
  - [Boundaries that are not negotiable here](#boundaries-that-are-not-negotiable-here)
  - [Before calling package work complete](#before-calling-package-work-complete)
  - [Drift handling in this package](#drift-handling-in-this-package)
  - [Update rules for this package](#update-rules-for-this-package)
...
|---|---|
| What is an eval, and what must it prove? | [references/01_overview.md](references/01_overview.md), then [SKILL.md](SKILL.md) |
| How do I write a suite? | [references/02_defining-evals.md](references/02_defining-evals.md), [templates/evals.yaml](templates/evals.yaml), [schemas/eval-definition.schema.json](schemas/eval-definition.schema.json) |
| What verdicts exist; when is one stale? | [references/03_verdicts-and-validity.md](references/03_verdicts-and-validity.md), [schemas/observation.schema.json](schemas/observation.schema.json) |
| What counts as falsifiability evidence? | [references/04_falsifiability.md](references/04_falsifiability.md), [schemas/falsifiability.schema.json](schemas/falsifiability.schema.json) |
| How is a report derived? | [references/05_reporting.md](references/05_reporting.md), [scripts/render_report.py](scripts/render_report.py) |
...
| What have consuming projects taught this package, and did it land? | [ledger.jsonl](ledger.jsonl), then the files its `incorporated_in` names. `just check` reports any row not yet incorporated |

### Boundaries that are not negotiable here

- **The package does not execute an eval.** Observations come from the consuming project's tools or an operator. A request for a scheduler, runner, retry policy or assertion DSL is out of scope —
  stop and say so.
- **History is append-only; reports are derived.** Never edit an `examples/*/history.jsonl` line or an `examples/*/report.md` by hand. Regenerate the report; supersede the record.
- **Negative fixtures must keep failing.** `examples/validation-failures/` exists to prove the validator still bites. Run `just validate-negatives` after any validator change.
- **Scripts stay stdlib-only.** `PyYAML` is optional convenience, never a requirement.

### Before calling package work complete

```bash
just runtimes         # confirm the pinned venv, not the host interpreter
just validate-all     # template, example suites, negative fixtures
just check            # governance coherence assertions
```

### Drift handling in this package

The generic procedure is in [Drift handling](#drift-handling) above. In this repo the common conflicts have known winners: a `report.md` that disagrees with its `history.jsonl` is stale output —
regenerate it, never edit it. A document that disagrees with
`schemas/*.json` about the data contract is wrong; the schema is the contract. A `SCRATCHPAD.md` claim that disagrees with `CHANGELOG.md` about what shipped loses.

### Update rules for this package

These are additional to the generic [Update rules](#update-rules) above.

| Change type | Update |
|---|---|
| Method or definition semantics changed | `references/`, then `schemas/` if the contract moved, then `scripts/validate_suite.py` |
| Validator rule added or changed | `scripts/validate_suite.py`, a negative fixture under `examples/validation-failures/`, and its README row — then run `just validate-negatives` |
| A suite or history changed | Regenerate the matching `report.md` with `scripts/render_report.py`; `just check` fails if you forget |
| Script added, removed, or repurposed | `scripts/README.md` (purpose, inputs, outputs, safety, idempotency) and the safety-label table |
| New invariant worth enforcing | Extend a registry in `scripts/check_governance.py`, and prove the new check can fail |
| Version bumped | `SKILL.md` frontmatter, `README.md`, and the `CHANGELOG.md` heading — all three, one pass |
| Routing changed | This file and `context-map.yaml` together |
| Durable decision or rule settled | Propose it per `.archcore/index.guide.md` "Proposing another document"; only `promote` mode writes `.archcore/` content, and the index must gain a row |
| Anything durable | Append `CHANGELOG.md` |

### Answer contract addition

Beyond the generic [Agent answer contract](#agent-answer-contract): this package makes claims testable, so do not make an untested claim about it. If `just check` or `just validate-all` has not been
